...
class FlowerClient(NumPyClient):
    ...
    @staticmethod
    def write_many_to_json(clients: list["FlowerClient"], path, filename):
        full_path = os.path.join(path, filename)
        with open(full_path, mode="w") as file:
            json.dump(
                {client.cid: client.__dict__ for client in clients}, file, indent=4
            )

    # @classmethod
    # def generate_many(cls, num) -> list["FlowerClient"]:
